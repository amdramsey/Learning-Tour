        ...
        signBtn.setOnClickListener{
            if(!mailTxt.text.toString().trim().isEmpty()) {
                if(!passTxt.text.toString().trim().isEmpty()) {
                    ...
                    if(passTxt.text.toString().trim() == cpassTxt.text.toString().trim()) {login();clearAll();showMainActivity(userName)}else{Toast.makeText(this, "Password doesn't not match")}
        ...
        db.addValueEventListener(object: ValueEventListener {
            ...
            override fun onDataChange(p0: DataSnapshot) {
                ...
                if(checkUAP()) {
                    ...
                    Toast.makeText(this@LoginActivity, "Signed in :)")
                    clearAll()
